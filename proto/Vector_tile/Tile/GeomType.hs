{-# LANGUAGE BangPatterns, DataKinds, DeriveDataTypeable, DeriveGeneric, FlexibleInstances, MultiParamTypeClasses, OverloadedStrings
 #-}
{-# OPTIONS_GHC  -w #-}
module Vector_tile.Tile.GeomType (GeomType(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum GeomType where
  ...
  fromEnum POLYGON = 3
  toEnum = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Vector_tile.Tile.GeomType") . toMaybe'Enum
  succ UNKNOWN = POINT
  succ POINT = LINESTRING
  succ LINESTRING = POLYGON
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Vector_tile.Tile.GeomType"
  pred POINT = UNKNOWN
  pred LINESTRING = POINT
  pred POLYGON = LINESTRING
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Vector_tile.Tile.GeomType"
