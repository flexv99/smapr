...
module Proto.Vector_tile.Tile (Tile (..)) where
...
instance P'.Mergeable Tile where
  mergeAppend (Tile x'1 x'2) (Tile y'1 y'2) =
        ...
        case wire'Tag of
          ...
          _ ->
            let (field'Number, wire'Type) = P'.splitWireTag wire'Tag
             in if Prelude'.or [16 <= field'Number && field'Number <= 8191]
...
instance P'.ReflectDescriptor Tile where
  ...
  reflectDescriptorInfo _ =
    Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".vector_tile.Tile\", haskellPrefix = [], parentModule = [MName \"Vector_tile\"], baseName = MName \"Tile\"}, descFilePath = [\"Vector_tile\",\"Tile.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".vector_tile.Tile.layers\", haskellPrefix' = [], parentModule' = [MName \"Vector_tile\",MName \"Tile\"], baseName' = FName \"layers\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 3}, wireTag = WireTag {getWireTag = 26}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = True, mightPack = False, typeCode = FieldType {getFieldType = 11}, typeName = Just (ProtoName {protobufName = FIName \".vector_tile.Tile.Layer\", haskellPrefix = [], parentModule = [MName \"Vector_tile\",MName \"Tile\"], baseName = MName \"Layer\"}), hsRawDefault = Nothing, hsDefault = Nothing, isMapField = False, mapKeyVal = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [(FieldId {getFieldId = 16},FieldId {getFieldId = 8191})], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False, jsonInstances = False, mapEntry = False}"
...
instance P'.TextMsg Tile where
  ...
  textGet =
    ...
    where
      parse'layers = Prelude'.fmap (\v o -> o{layers = P'.append (layers o) v}) (P'.try (P'.getT "layers"))
